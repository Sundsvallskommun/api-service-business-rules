...
public class DriverNewParkingPermitRule implements ParkingPermitRule {
	...
	private List<String> validateInput(List<Fact> facts) {
		...
		if (isNull(walkingAbility) || !isValidDurationEnumValue(disabilityDuration.getValue())) {
		...
		if (isNull(walkingAbility) || !walkingAbility.hasBooleanValue()) {
		...
		if ((nonNull(walkingAbility) && toBoolean(walkingAbility.getValue())) &&
			(isNull(disabilityWalkingDistanceMax) || !disabilityWalkingDistanceMax.hasNumericValue())) {
		...
		if (isNull(stakeHoldersApplicantPersonId) || !isValidUuid(stakeHoldersApplicantPersonId.getValue())) {
