...
import static se.sundsvall.businessrules.rule.parkingpermit.enums.DurationEnum.LESS_THAN_6_MONTHS;
import static se.sundsvall.businessrules.rule.parkingpermit.enums.ParkingPermitFactKeyEnum.APPLICATION_RENEWAL_CHANGED_CIRCUMSTANCES;
...
public class DurationCriteria implements Criteria {
	...
	private CriteriaResult evaluateRenewalApplication(final boolean changedCircumstances, final String disabilityDuration) {
		if (changedCircumstances && LESS_THAN_6_MONTHS.isEqualTo(disabilityDuration)) {
			return new CriteriaResult(false, DURATION_LESS_THAN_THRESHOLD_VALUE);
		}

		return new CriteriaResult(true, LESS_THAN_6_MONTHS.isEqualTo(disabilityDuration) ? RENEWAL_WITH_UNCHANGED_CIRCUMSTANCES : DURATION_MORE_THAN_THRESHOLD_VALUE);
	}
	...
	private CriteriaResult evaluateNewApplication(final String disabilityDuration) {
		...
		return new CriteriaResult(true, DURATION_MORE_THAN_THRESHOLD_VALUE);
	}
}
